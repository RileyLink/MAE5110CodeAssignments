import numpy as np
def rk4(timestep, sim_time, x0, dynamics, params, check_event=lambda state, params: (False, state)):
    ...
    for step, t in enumerate(time_traj[:-1]):
        ...
        events[step+1], state_traj[:, step+1] = check_event(state_traj[:, step] + (timestep/6) * (k1+2*k2+2*k3+k4),params)
